...
import { ChevronLeft, Info, HelpCircle, Zap, TrendingUp, DollarSign, CheckCircle } from 'lucide-react';
...
const GameQuizDetailPage = () => {
  ...
  useEffect(() => {
    // Note: Match detail page usually shows a quiz details for that match
    // For now, using getQuizById since id here is quiz_id
      ...
      <div className="container" style={{ padding: '1.5rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        {match.effective_banner_url && (
          <div style={{ 
            ...
            <div style={{
              ...
              background: `url("${match.effective_banner_url}") center/cover no-repeat`,
            ...
            <div style={{
              ...
              width: '100%',
              height: '100%',
              background: `url("${match.effective_banner_url}") center/contain no-repeat`,
            }}></div>
          </div>
        )}
        ...
        <div style={{ padding: '2.5rem 0' }}>
          {match.is_submitted ? (
            <button 
              className="btn-elite" 
              style={{ width: '100%', height: '72px', fontSize: '1.25rem', background: '#10b981', color: 'white', border: 'none', boxShadow: '0 20px 40px -10px rgba(16, 185, 129, 0.3)' }}
              onClick={() => navigate(`/game-result/${id}`)}
            >
              View Result <CheckCircle size={24} style={{ marginLeft: '0.75rem' }} />
            </button>
          ) : (
            <button 
