...
const StudyReviewPage = () => {
  ...
  // Real data passed from StudyQuestionPage via navigate state
  ...
  const handleFinalSubmit = () => {
    // Navigate back to StudyQuestionPage with a signal to submit
    navigate(`/study-quiz-play/${realQuizId}`, {
      state: { triggerSubmit: true, answers, questions }
    });
  };

  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#0a192f' }}>
      <div className="container" style={{ padding: '1.5rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
          ...
          <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#ffffff', margin: 0 }}>Review Submission</h1>
        </div>
        ...
        <div style={{ marginTop: 'auto', paddingBottom: '2rem' }}>
          <div style={{ padding: '1.25rem', marginBottom: '1.5rem', textAlign: 'center', border: '1px solid rgba(56,189,248,0.2)', background: 'rgba(56,189,248,0.05)', borderRadius: '1rem' }}>
            <p style={{ fontSize: '0.85rem', color: '#94a3b8', fontWeight: 600, lineHeight: '1.6' }}>
              {unanswered > 0
                ? `⚠️ You have ${unanswered} unanswered question${unanswered > 1 ? 's' : ''}. You can still go back and answer them.`
                : '✅ All questions answered! Your final score will be calculated upon submission.'}
            </p>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            ...
            <button
              ...
              style={{ flex: 2, background: 'linear-gradient(135deg, #38bdf8, #0284c7)', border: 'none', color: '#ffffff', padding: '1rem', borderRadius: '0.75rem', fontWeight: 800, fontSize: '0.95rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', boxShadow: '0 4px 15px rgba(56,189,248,0.35)' }}
            >
